...
namespace ApiHelper.Client
{
    public abstract class ClientBase
    {
        ...
        protected async Task<TResponse> DeleteDecodedContent<TResponse, TContentResponse>(string uri, params KeyValuePair<string, string>[] requestParameters)
   where TResponse : ApiResponse<TContentResponse>, new()
        {
            using (var apiResponse = await ApiClient.DeleteEncodedContent(uri, requestParameters))
            {
                return await DecodeJsonResponse<TResponse, TContentResponse>(apiResponse);
            }
        }
        ...
        protected async Task<TResponse> PutEncodedContentWithSimpleResponse<TResponse, TContentResponse, TModel>(string url, TModel model)
where TModel : ApiModel
where TResponse : ApiResponse<TContentResponse>, new()
        {
        ...
        private static async Task<TResponse> DecodeJsonResponse<TResponse, TDecode>(HttpResponseMessage apiResponse) where TResponse : ApiResponse<TDecode>, new()
        {
            ...
            var deserialized = JsonConvert.DeserializeObject(response.ResponseResult);   
            response.Data = Json.Decode<TDecode>(deserialized.ToString()); 
            return response;
        }
    }
}
